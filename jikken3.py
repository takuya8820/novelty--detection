# -*- coding: utf-8 -*-
"""
Created on Tue Dec 18 12:22:15 2018
...
encoderR_train_fake = encoderR_train + np.random.normal(0,noiseSigma,encoderR_train.shape)
decoderR_train = decoderR(encoderR_train_fake, z_dim_R, keepProb=1.0)
...
for ite in range(15000):
    ...
    if ite % 500 == 0:
        ...
        for ind, testFakeRatio in enumerate(testFakeRatios):
            ...
            if ind == 0:
                ...
                for figInd in np.arange(figInds.shape[1]):
                    ...
                    path = os.path.join(jikkenvisualPath,"img_test_true_{}_{}_{}_{}.png".format(postFix,noiseSigma,testFakeRatio,ite))
                    plt.savefig(path)
				#--------------
                ...
                for figInd in np.arange(figInds.shape[1]):
                    ...
                    path = os.path.join(jikkenvisualPath,"img_test_fake_{}_{}_{}_{}.png".format(postFix,noiseSigma,testFakeRatio,ite))
                    plt.savefig(path)
				#--------------
        ...
        saver.save(sess,"./models_jikken3/model{}.ckpt".format(postFix))
		#--------------
