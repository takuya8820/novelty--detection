...
if len(sys.argv) > 1:#sys.argvには、コマンドで”python ’ファイル名’　〇〇”と打つと['ファイル名'、〇〇]というリストが格納されます
                     #要はコマンドで実行するときのpython以下のスペースで区切ったものが配列になります
    # 文字の種類
	targetChar = int(sys.argv[1]) #sys.argv[0]は必ずファイル名なので、コマンドで数を指定してあげた部分を参照したいときはsys.argv[1]になります。この場合１つ目に書いたものがtargetCharに入ります

	# trail no.
	if len(sys.argv) > 2:#コマンドの引数がファイル名をのぞいて2個以上書いたとき、2個目が何かをtrialNoに格納するようになってます。
		trialNo = int(sys.argv[2])
	else:#コマンドの引数が２個のときなので、コマンドでファイル名以外に何か１つだけ書くと、強制的にtrialNoは１になります
		trialNo = 1
...
for ite in range(30000):
	...
	# ノイズを追加する(ガウシアンノイズ)
	# 正規分布に従う乱数を出力
	batch_x_fake = batch_x + np.random.normal(0,noiseSigma=i,batch_x.shape)
	#--------------
